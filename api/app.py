...
CLOCK_PICTURE       = "static/images/clock/clock.png"
CLOCK_IMAGE_FOLDER  = "static/images/clock/"
...
def home():
    # if request.method == "POST":
        ...
        old_clock_image = f"{CLOCK_IMAGE_FOLDER}clock.png"
        ...
        for image in files:
            ...
            if(len(faces) == 1):
                ...
                for i, (x, y, w, h) in enumerate(faces):
                    ...
                    if(count == 1):
                        new_clock_image = old_clock_image
                    else:
                        new_clock_image = f"{CLOCK_IMAGE_FOLDER}{name}.png"

                    print(f'count: {count}, new_clock_image : {new_clock_image}')

                    clock       = Image.open(new_clock_image)
                    back_im     = clock.copy()
                    back_im.paste(img, centre, mask_im)

                    new_clock_image = f"{CLOCK_IMAGE_FOLDER}{name}.png"
                    back_im.save(new_clock_image)

                    print(f'saving as {new_clock_image}')
